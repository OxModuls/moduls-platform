import { ArrowLeftRight, ArrowUpDown, BadgeDollarSign, Wallet } from "lucide-react";
...
const AgentTradeTab = ({
  ...
  const modulsSalesManagerAddress = chainId === 1328 ? config.contractAddresses.testnet.modulsSalesManager : chainId === 1329 ? config.contractAddresses.mainnet.modulsSalesManager : config.contractAddresses.testnet.modulsSalesManager;
  
  // Use the hook for additional functionality
  const { getTokenAmountForEther,  } = useModulsSalesManager(token.contractAddress);
...
const buyCostDataQueryEnabled = !!currentAmount && activeTradeTab === "buy" && !!token.contractAddress && !!connectedAddress 

const sellReturnDataQueryEnabled = !!currentAmount  && activeTradeTab == "sell" && !!token.contractAddress && !!connectedAddress && amountType === "tokens"
...
console.log("Current Amount", currentAmount, "Parsed Current Amount", parsedCurrentAmount)
  ...
  const { data: sellReturnData } = useReadContract({
    ...
    args: [token.contractAddress, parsedCurrentAmount   ],
    query: {
      enabled: sellReturnDataQueryEnabled && amountType === "tokens",
      refetchInterval: 10000
    }
  });
  ...
  const calculatedSellReturn = sellReturnData ? formatEther(sellReturnData) : null;
  ...
  const checkAndHandleApproval = async (tokenAmount) => {
    if (!tokenAmount || !connectedAddress || !token.contractAddress || !modulsSalesManagerAddress) {
    ...
    try {
      ...
      toast.info("Approval needed. Please approve the transaction in your wallet.");
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    if (activeTradeTab === "buy") { 
        if(amountType === "tokens" && calculatedBuyCost) {
            ...
            const maxCost = totalCostWei + (totalCostWei * BigInt(values.slippage) / BigInt(100));
  ...
  const handleAmountChange = async (value, setFieldValue) => {
    ...
    if (amountType === "tokens") {
      ...
      adjustedValue = value === "" ? "" : intValue.toString();
    }
    ...
    if (activeTradeTab === "buy") {
      ...
      // For sell, always use token amount
      setCurrentAmount(adjustedValue);
      setConvertedAmount(0); // No conversion needed for sell
    }
  ...
  const handleMaxClick = async (setFieldValue) => {
    if (activeTradeTab === "buy") {
      if (amountType === "sei") {
        ...
        setFieldValue("amount", maxTokens);
      ...
      const maxAmount = tokenBalanceData ? formatEther(tokenBalanceData) : "0";
      setFieldValue("amount", maxAmount);
      handleAmountChange(maxAmount, setFieldValue);
      setConvertedAmount(0);
    }
      ...
      .test("integer", function(value) {
        if (activeTradeTab === "buy" && amountType === "tokens") {
          if (!Number.isInteger(Number(value)) || Number(value) <= 0) {
            return this.createError({
              message: "For buying tokens, you must enter a whole number greater than 0.",
            });
          }
        }
        return true;
      ...
      .test("insufficient-balance", function(value) {
        ...
        if (activeTradeTab === "buy") {
          if (amountType === "sei") {
            ...
            if (parseFloat(value) > parseFloat(formatEther(balanceData.value))) {
              return this.createError({
            ...
            if ( parseFloat(calculatedBuyCost.totalCost) > parseFloat(formatEther(balanceData.value))) {
              return this.createError({
                message: "You do not have enough SEI to complete this trade.",
              });
            }
          ...
          if (parseFloat(value) > parseFloat(formatEther(tokenBalanceData))) {
            return this.createError({
              message: "You do not have enough tokens to complete this trade.",
            });
          }
        }
        return true;
      ...
      {!token.isAddressValid ? (
        ...
        <div className="pt-4 text-center">
          <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            <div className="flex items-center gap-2 mb-2 justify-center">
              ...
              <h3 className="font-medium text-yellow-600 dark:text-yellow-400">Token Not Registered</h3>
            </div>
            <p className="text-sm text-yellow-600/80 dark:text-yellow-400/80">
              This token is not yet registered for trading on the ModulsSalesManager contract.
            </p>
          </div>
        </div>
        ...
        <div className="pt-4 text-center">
          <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
            <div className="flex items-center gap-2 mb-2 justify-center">
              <BadgeDollarSign className="size-4 text-red-600 dark:text-red-400" />
              <h3 className="font-medium text-red-600 dark:text-red-400">Trading Paused</h3>
            </div>
            <p className="text-sm text-red-600/80 dark:text-red-400/80">
            ...
            {({ values, setFieldValue, isValid, dirty }) => (
              <Form className="mt-3 px-2 py-4 bg-neutral-850 border rounded-lg flex flex-col gap-4">
                ...
                <div className="w-full flex flex-col gap-1">
                  ...
                  {activeTradeTab === "buy" && amountType === "sei" && convertedAmount > 0 && (
                    <div className="text-xs text-neutral-400 text-center">
                      ≈ {convertedAmount.toFixed(6)} {agent?.tokenSymbol?.toUpperCase() || token.name.toUpperCase()}
                    </div>
                  )}
                
                </div>
                
                {activeTradeTab === "buy" && calculatedBuyCost && amountType === "tokens" && (
                  <div className="p-3 bg-green-500/5 border border-green-500/10 rounded-lg">
                    <div className="flex justify-between text-sm">
                      <span>Cost:</span>
                      <span>{parseFloat(calculatedBuyCost.cost).toFixed(12)} SEI</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Tax:</span>
                      <span>{parseFloat(calculatedBuyCost.tax).toFixed(12)} SEI</span>
                    </div>
                    <div className="flex justify-between text-sm font-medium">
                      <span>Total:</span>
                      <span>{parseFloat(calculatedBuyCost.totalCost).toFixed(12)} SEI</span>
                    </div>
                  </div>
                )}

                {activeTradeTab === "buy" && calculatedBuyCostWithSEI && amountType === "sei" && (
                  <div className="p-3 bg-green-500/5 border border-green-500/10 rounded-lg">
                    <div className="flex justify-between text-sm">
                      <span>Cost:</span>
                      <span>{parseFloat(calculatedBuyCostWithSEI.cost).toFixed(12)} SEI</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Tax:</span>
                      <span>{parseFloat(calculatedBuyCostWithSEI.tax).toFixed(12)} SEI</span>
                    </div>
                    <div className="flex justify-between text-sm font-medium">
                      <span>Total:</span>
                      <span>{parseFloat(calculatedBuyCostWithSEI.totalCost).toFixed(12)} SEI</span>
                    </div>
                  </div>
                )}
                ...
                {activeTradeTab === "sell" && calculatedSellReturn && (
                  <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-lg">
                    <div className="flex justify-between text-sm font-medium">
                      ...
                      <span>{parseFloat(calculatedSellReturn).toFixed(12)} SEI</span>
                    </div>
                  </div>
                )}
                ...
                <div>
                  <button
                    type="submit"
                    disabled={
                      (activeTradeTab === "buy" ? buyTokenStatus.isPending : sellTokenStatus.isPending) || 
                      isApprovalPending ||
                      !connectedAddress || 
                      !isValid || 
                      !dirty || 
                  ...
                  >
                    {((activeTradeTab === "buy" ? buyTokenStatus.isPending : sellTokenStatus.isPending) || isApprovalPending) && (
                        ...
                        <>
                          {amountType === "sei" ? (
                            <>
                              You will receive{" "}
                              <span>
                                {convertedAmount > 0 ? convertedAmount.toFixed(6) : "0"}
                              </span>{" "}
                              {agent?.tokenSymbol?.toUpperCase() || token.name.toUpperCase()}
                            </>
                          ) : (
          ...
          {token.tradeStats ? (
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{token.tradeStats.totalBuys}</div>
                <div className="text-sm text-muted-foreground">Total Buys</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{token.tradeStats.totalSells}</div>
                <div className="text-sm text-muted-foreground">Total Sells</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold">{parseFloat(token.tradeStats.totalVolumeETH).toFixed(12)} SEI</div>
                <div className="text-sm text-muted-foreground">Total Volume</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold">{parseFloat(token.tradeStats.totalVolumeToken).toLocaleString()}</div>
